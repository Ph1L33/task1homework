...
# #overwrite
# resource "google_storage_bucket" "itphil231" {
#   name          = "${var.project_id}-itphil231"
...
#overwrite
resource "google_storage_bucket" "itphil231" {
  name          = "${var.Itphil231[0]}-itphil231"
  location      = var.Itphil231[4]
  force_destroy = true
  website {
    main_page_suffix = "index.html"
    not_found_page   = "404.html"
...
resource "google_storage_bucket_access_control" "public_rule" {
  bucket = google_storage_bucket.itphil231.name
  role   = "READER"
  entity = "allUsers"
}
...
resource "google_storage_bucket_object" "default" {
  ...
  bucket       = google_storage_bucket.itphil231.id
}
